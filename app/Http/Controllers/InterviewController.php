...
PROMPT;
        ...
        try {
            // Choose provider: Prefer Anthropic/Claude when configured, otherwise OpenAI
            $aiProvider = env('AI_PROVIDER', null);
            if (!$aiProvider) {
                // If an Anthropic key exists, prefer Anthropic
                $aiProvider = env('ANTHROPIC_API_KEY') ? 'anthropic' : 'openai';
            }
            ...
            // Try Anthropic (Claude Haiku) first when requested
            if ($aiProvider === 'anthropic') {
                $anthropicKey = env('ANTHROPIC_API_KEY');
                if (!$anthropicKey) {
                    Log::warning('ANTHROPIC_API_KEY not set, falling back to OpenAI');
                } else {
                    $anthropicModel = env('ANTHROPIC_MODEL', 'claude-haiku-4.5');
                    Log::info('Attempting to generate questions via Anthropic', [
                        'model' => $anthropicModel,
                        'skill_count' => count($skills),
                        'num_questions' => $count
                    ]);

                    $anthropicResp = Http::withHeaders([
                        'x-api-key' => $anthropicKey,
                        'Content-Type' => 'application/json'
                    ])->timeout(60)->post('https://api.anthropic.com/v1/complete', [
                        'model' => $anthropicModel,
                        // Anthropic expects a Human/Assistant style prompt
                        'prompt' => "Human: {$prompt}\n\nAssistant:",
                        'max_tokens' => 1000,
                        'temperature' => 0.7,
                    ]);
                    ...
                    Log::info('Anthropic Response', [
                        ...
                        'successful' => $anthropicResp->successful(),
                        'body_preview' => substr($anthropicResp->body(), 0, 500)
                    ]);

                    if ($anthropicResp->successful()) {
                        $anthropicData = $anthropicResp->json();
                        // Anthropic returns the generated text under 'completion' in many SDKs
                        $content = $anthropicData['completion'] ?? ($anthropicData['response'] ?? null);
                        if (is_null($content)) {
                            // As a last resort, try to use the raw body
                            $content = $anthropicResp->body();
                        }
                    } else {
                        Log::error('Anthropic API error', [
                            'status' => $anthropicResp->status(),
                            'body' => $anthropicResp->body()
                        ]);
                        // continue to fallback to OpenAI below
                    }
                }
            }

            // If we didn't get content from Anthropic, try OpenAI as before
            if (is_null($content)) {
                ...
                Log::info('Attempting to generate questions via OpenAI', [
                    'model' => $model,
                    'skill_count' => count($skills),
                    'num_questions' => $count
                ]);
            ...
            // Determine which model string we used (for logging)
            $usedModel = ($aiProvider === 'anthropic')
            ...
            if (!is_array($questions) || empty($questions)) {
                Log::warning('Invalid questions format from AI provider', [
                    'content' => $content,
                    'model' => $usedModel,
                    'decoded' => $questions
                ]);
